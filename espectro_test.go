...
func TestProcessAudio(t *testing.T) {
	...
	err = ProcessAudio(f, os.Stdout, 41100, 3 * time.Second, 0)
	if err != nil {
		t.Error(err)
	}
}
...
func TestProcessSignal(t *testing.T) {
	for i, tt := range []struct {
		...
		out := processSignal(tt.signal)
		outLen := len(out)
		if oL := len(tt.signal) / 2; outLen != oL {
			t.Errorf("Signal %d: length of output should equal %d, got %d", i, oL, outLen)
			continue
		}
		if wantedLen := len(tt.wantedOut); outLen != wantedLen {
			t.Errorf("Signal %d: Wanted output and processed output should have the same length; wanted %d, got %d", i, wantedLen, outLen)
			continue
		}
...
func TestAverage(t *testing.T) {
	...
	// handle channels = 0
	out = average(spectrum, 0)
	if len(out) != len(spectrum) {
		t.Error("output should be the same lenght as input when channels is 0")
	}
}
...
func TestSignalFromBytes(t *testing.T) {
	for _, tt := range []struct {
		...
		signal := signalFromBytes(tt.rawData)
...
func TestBufferFromDuration(t *testing.T) {
	for _, tt := range []struct {
		sampleRate   int
		...
		chunk := bufferFromDuration(tt.duration, tt.sampleRate)
		if size := len(chunk); size != tt.expectedSize {
			t.Errorf("wanted %d, got %d", tt.expectedSize, size)
		}
	}
}